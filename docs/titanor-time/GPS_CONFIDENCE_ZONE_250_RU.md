...
| Наблюдение логов 21:58–22:13 UTC (~15 мин) | app: **0** строк Error/Exception/Prisma/SQL; scheduler: **13 тиков ok, 0 failed**; `/api/ready` 200 всё время; контейнер healthy |
| Данные не изменены (проверка на 22:13 UTC, ~15 мин после свопа) | prod `AttendanceException` md5 **`1d0a29c53d0e3a08b1caf52aa8f4c777`** и `AuditEvent(id/type/entity/createdAt)` md5 **`563b8999b0bc705ae4d8c5785d0e852f`** — **байт-в-байт как до свопа**; все 76 счётчиков строк без изменений; **0 строк создано** после T0 (ClockEvent/ClockEventLocation/AttendanceException/AuditEvent/Timesheet/WorkerDeviceInstallation); `ATTENDANCE_POLICY_UPDATED` count = **0**; никаких тестовых GPS-событий (сейчас ≈00:00 Helsinki — организической активности нет) |
