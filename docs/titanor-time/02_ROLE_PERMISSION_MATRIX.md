...
| `worker.profile.read.own` | `WORKER` | собственный `EmployeeProfile`/`EmployeeQualification` | — | нет | нет | — |
| `worker.profile.update.own` | `WORKER` | собственные поля профиля/фото + свои карточки квалификации | требует `version` на `EmployeeProfile`; не даёт доступ к договору | нет | нет | нет |
| `worker.profile.read.all` | `ADMIN`, `SUPER_ADMIN` | любой работник, включая договор | — | нет | нет | — |
| `worker.profile.update.all` | `ADMIN`, `SUPER_ADMIN` | любой работник: поля профиля/фото/карточки + прикрепление/замена договора | требует `version` на `EmployeeProfile` | нет | да, для договора | нет |
...
| `attendance.gps.read.raw` | `ADMIN`, `SUPER_ADMIN` | retained Check In/Out coordinates конкретного worker | отдельные API/page; `private, no-store`; максимум 31 день/200 событий; FOREMAN/WORKER — 0 grants; overview координат не содержит | нет | да (`ATTENDANCE_RAW_GPS_VIEWED`, без координат) | нет |
...
| `timesheet.scope_review.all` | `ADMIN`, `SUPER_ADMIN` | любой `TimesheetReviewScope`, включая любой `SITE`, `NON_SITE(DATA)` и `NON_SITE(EMPTY_FALLBACK)` | основной гарантированный путь review: ADMIN может approve/return независимо от наличия или назначения FOREMAN; для `NON_SITE` это единственный путь; те же раздельные approve/return precondition, что `timesheet.foreman_review` | нет | да | — |
...
| `timesheet.final_approve` | `ADMIN`, `SUPER_ADMIN` | вся компания | чистый переход `FOREMAN_APPROVED → FINAL_APPROVED`, не меняет данные; `FOREMAN_APPROVED` — legacy-имя состояния «все scope подтверждены», оно не требует действия пользователя с ролью FOREMAN | нет | да | да |
