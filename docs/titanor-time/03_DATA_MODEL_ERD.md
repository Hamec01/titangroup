...
**ExportBatch** (immutable) — `id`, `periodId FK`, `format enum (CSV_V1)`, `createdByUserId FK`,
`fileHash varchar`, `rowCount int`, `correctsBatchId FK → ExportBatch` (self, да), `createdAt`.

**ExportItem** (immutable) — `id`, `exportBatchId FK`, `employeeId FK`, `timesheetVersionId FK`,
`regularMinutes`, `additionalMinutes`, `dailyOvertime50Minutes`, `dailyOvertime100Minutes`,
`weeklyOvertimeMinutes`, `sundayMinutes`, `nightMinutes`, `holidayMinutes`, `travelMinutes` (все
