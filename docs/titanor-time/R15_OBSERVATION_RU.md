...
- **Статус (2026-09-04):** прод здоров (`titanor-time-app:d7f-d216482`, healthy, restart 0, schema
  100/100). Образы Deploy A→F **физически развёрнуты 2 – 3 сентября** (каждый swap был отдельно
  разрешён владельцем). **Указание владельца 2026-09-04:** Deploy F **и** запись «Что нового» в
  `/guide` **НЕ считать production-live** до отдельного отчёта о production-деплое и его ревью
  владельцем. Статус = «развёрнут, не закрыт». Production без явного разрешения не менять; тестовых
  данных на prod не создавать.
  - Отдельный отчёт о production-деплое Deploy F: **`R15_D7_DEPLOY_F_PROD_REPORT_RU.md`** (черновик,
    ждёт ревью владельца).
  - Запись `/guide` «2–3 сентября 2026» (коммит `c76d439`) — в работающем образе её НЕТ; станет
    видна только после отчётного деплоя приложения.
- **Полный R15 owner sign-off ещё НЕ получен** — открыто 5 P1-пунктов из финального аудита
  `fixroad.md` (см. §«Финальный аудит»). Ранее (2026-09-03) владелец дал только **технический**
  sign-off по коду/деплоям D7 A→F — это не закрытие релиза.
