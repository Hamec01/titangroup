...
| D1b | **Нет «удалить работника в архив».** Владелец хочет отдельное действие «убрать работника из системы с сохранением данных» (создан по ошибке / дубль / тест) — отличное от «деактивировать» (уволен). Физического DELETE у Employee нет by design (`T9_INTERNAL_TEST_PLAN.md` §1 — снёс бы payroll-историю). | P2 | **открыто.** Варианты: (a) `Employee.archivedAt` миграция + фильтр в ~6–8 списках/пикерах + вкладка «Архив» + «восстановить из архива»; (b) без миграции — список работников по умолчанию скрывает неактивных, галочка «показать архив», а «Деактивировать» и есть «в архив». Нужен выбор владельца (a бо́льшая, с prod-миграцией #99). |
