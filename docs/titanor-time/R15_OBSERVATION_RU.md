...
- **Статус (2026-09-04 ~15:43 UTC):** прод здоров (`titanor-time-app:d7f-fd8494c`, healthy,
  restart 0, schema 100/100). Образы Deploy A→F развёрнуты 2 – 3 сентября (каждый swap отдельно
  разрешён владельцем); **владелец подтвердил Deploy F технически live**, затем разрешил 2-й
  web-only swap.
  - Отдельный отчёт о production-деплое Deploy F: **`R15_D7_DEPLOY_F_PROD_REPORT_RU.md`** (§7 = 2-й swap).
  - **Запись `/guide` «2–3 сентября 2026» (`c76d439`) и правка F03 (переключатель «часто нет GPS» в
    пояснительном режиме, `2fa0d5d`+`e5b1a42`) — LIVE** с 2026-09-04 ~15:43 UTC (web-only swap
    `d7f-d216482`→`d7f-fd8494c`, без миграции, простой ≈ 2.8 c, релизный прогон 19/0/2 skip +
    worker-dossier-qa 31/0 + restart-persistence 5/0+18/0 + db 64/0 + unit 18/0, подтверждено на
    живом `/guide`). Rollback-контейнер `titanor-time-prod-app-pre-fd8494c` (образ `d7f-d216482`) сохранён.
...
- **Держать до конца периода наблюдения (по слову владельца):** rollback-контейнер
  `titanor-time-prod-app-pre-fd8494c` (образ `d7f-d216482`) + backup `production-20260904T154045Z-pre-deploy`
  (on+off-box) — активный rollback для текущего prod-образа `d7f-fd8494c`. Более ранние rollback-контейнеры
  (`…-pre-d216482` = `d7e-5cce319`, и старше) остаются под общей заморозкой ниже, не активный rollback-путь.
