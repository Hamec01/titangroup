...
| D6 | **«Рабочая зона» вводит начальника в ступор** — на практике это заказчик (несколько заказчиков на одном объекте). | P3 (терминология) | ✅ **Задеплоено 2026-09-02** — `496aa3c` / `customer-rename-496aa3c`, отчёт `CUSTOMER_RENAME_DEPLOY_RU.md`. «Рабочая зона» → «Заказчик» / «Work area» → «Customer» во всей админке, инструкции, CSV/PDF-экспорте. GPS-тексты у рабочего переформулированы («территория объекта» / «На объекте»/«Вне объекта»), т.к. геозона на весь объект, а не на заказчика. Модель БД `WorkArea` и роуты не тронуты. Без миграции. Browser lane 105/84/26/33 + clock-panel 55/55. |
