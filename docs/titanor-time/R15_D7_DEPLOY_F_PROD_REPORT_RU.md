...
> Образ `titanor-time-app:d7f-d216482` развёрнут на production 2026-09-03 19:34Z (web-only swap,
> отдельно разрешённый владельцем — см. §1). **Владелец подтвердил: Deploy F можно считать
> технически live.** Запись «Что нового» в `/guide` (`c76d439`) и правка F03 — **не в работающем
> образе**, поедут следующим деплоем (§6 п.2).
>
> **Полный R15 owner sign-off НЕ дан.** Остаются: реальная device-приёмка на Android (F02),
> восстановление backup публичного сайта (F04), процесс разбора открытых исключений начальником.
...
**Полный R15 owner sign-off НЕ дан** — открыто:

1. **F02 — минимальная ручная device acceptance на реальном Android** (за владельцем;
   browser-эмуляция Codex уже PASS). Тестовых часов на prod не создавать.
2. **F04 — восстановление `titanorgroup-backup.service`** (backup публичного сайта, не Titanor
   Time). Разрешён только первый root-этап — read-only диагностика по
   `R15_F04_PUBLIC_SITE_BACKUP_RUNBOOK_RU.md` §1; правки/перезапуск — только после показа точной
   причины и минимального плана. Titanor Time backup + GPS archive работают.
3. **Процесс разбора открытых исключений начальником** — ежедневный ответственный + SLA
   (`R15_ATTENDANCE_EXCEPTIONS_REVIEW_RU.md` §4 п.6). Исключения #1000 Andrei закрываются как
   «Тест» (DISMISS); по Meyer — админ принимает разом по фильтру объекта.

**Не в работающем образе — поедут СЛЕДУЮЩИМ деплоем приложения** (web-only swap `d7f-d216482` →
`d7f-<HEAD-sha>`, тот же паттерн, без миграции — только по отдельному разрешению владельца):
- запись «Что нового» `/guide` (`c76d439`);
- **F03** — переключатель «На объекте часто нет GPS-сигнала» в пояснительном режиме
  (`2fa0d5d` + `e5b1a42`; `R15_MEYER_GPS_FLAG_RU.md` / `R15_MEYER_GPS_AUTOACCEPT_PLAN_RU.md`).
  Галочку на Meyer владелец включит через UI **после** выпуска этой версии; автопринятие GPS —
  не включать.
