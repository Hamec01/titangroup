...
**Пилот на `titanor-time-app:t97-pilot-6a47ed3`, DB 98/98, app+scheduler `healthy` restarts 0,
`/api/ready` schema=current.** `deploy-6a47ed3.sh` (владелец): backup on+off-box (с
`gps-archive-manifest.json`), migrate 97→98 (0 failed, `GpsArchiveDay`+trigger+пусто), swap,
verify (`.runtime/gps-archive.cjs` bogus→2 + empty-key→3, retention `retentionOutcome:ok`, R07-A
регрессия, prod baseline). Rollback-контейнеры `-pre-6a47ed3` (на `t97-pilot-8724480`) —
**сохранены по указанию владельца**. Disposable-verify до деплоя: PASS 17/0 (§5a отчёта).
**Первый ручной прогон архива + systemd (владелец):** 5 sealable дней (2026-08-24…28) → written →
...
- **T16.1 (`4c1a06c`)** — на «Профиль» было два поля: старое свободное «Специальность» и новый
  блок «Профессии». Оставлено одно: поле «Специальность» убрано с экрана работника (partial PATCH
  его не шлёт → admin-значение сохраняется; админ по-прежнему правит на своей стороне), блок
  «Профессии» поднят наверх (туда, где была «Специальность»). В `EmployeeProfessionsEditor` убран
  отдельный переключатель «Добавить из» — теперь в самом `<select>` профессии последним пунктом
  «✎ Другая — вписать свою» (раскрывает поле названия + категорию).
- **T16.2 (`b4184ac`)** — «Действие требует внимания» больше не на экране часов: убраны баннер и
  статус-сообщение «одно или несколько действий требуют внимания»; статус-точка синхронизации не
  реагирует на terminal-fail (это не состояние синка). `describeErrorCode` → `lib/i18n/worker.ts`
  (`describeWorkerErrorCode`). Колокольчик теперь объединяет серверные уведомления (срок табеля) +
  `FAILED_TERMINAL` из outbox устройства — у каждой отклонённой отметки причина + «отметьтесь
  заново» + «Убрать» (`dismissFailedEvent`), обновляется по `subscribeOutboxChanges`.
  **Pull-to-refresh** (`PullToRefresh` в layout работника): тянуть страницу вниз от самого верха;
  после порога — отпустить ИЛИ держать ~2.5 с → приложение перезагружается (full navigation; SW
  network-first для `/worker`, новый деплой подхватывается без закрытия приложения). Не срабатывает
  пока открыт лист/меню (body-scroll lock).
- `tsc` + `next build` зелёные; регрессия `_test-offline-idb-invariants` 37/37,
  `_test-worker-notifications` 15/15, `_test-worker-professions` 17/17.
...
- В карточке уведомления «нужно утвердить» (и в drawer, и в тосте) под тегом «ПРЕДУПРЕЖДЕНИЕ» —
  маленькая строка с датой и временем: когда работник сдал или внёс правки. Источник —
  `timesheet.currentVersion.createdAt` (момент заморозки последней версии; поздняя правка сдвигает
  дату вперёд, а не показывает момент обнаружения алерта). Для уведомлений о допусках строки нет —
  там нет действия человека, и уже показан срок действия.
- `AdminNotificationView.eventAt: string | null`; `NotificationCenter` — `formatEventAt()` (локаль
  RU/EN, без «г.»), `.notif-drawer-item-time` / `.notif-toast-time` в `globals.css`.
- `_test-timesheet-approval-notifications` 14/14, `_test-qualification-notification-thresholds`
  35/35, `tsc` + `next build` зелёные. Схемы/миграций нет. Планировщик `t97-pilot-scheduler`
  тоже пересоздан на новом образе (на его работу изменение не влияет).

**`[2026-08-28]` `/guide` — раздел «Что нового» (`fe6f450`, пилот `t97-pilot-fe6f450`).**
- В конце страницы инструкции — датированный список изменений простым языком (без хешей и
  внутренних кодов задач), самое свежее сверху. Засеян работой 23–28 авг: авто-обед, единый
  экран утверждения, «Изменить часы», больничный из проверки, авто-закрытие смены, улучшения
  GPS, досье/допуски, сгруппированное меню.
- `lib/i18n/guide.ts` — новый `GuideChangeEntry` + `changelog` (RU + EN); `GuideView.tsx` —
  секция после «На что обратить внимание»; `globals.css` — `.guide-change-entry`. Кода приложения
  и схемы не трогали, миграций нет. `tsc` + `next build` зелёные.
- Ветка `feature/titanor-time-foundation` запушена в GitHub (`Hamec01/titangroup`).

**`[2026-08-28]` Авто-закрытие забытой смены через 16 ч (`85e0599`, пилот `t97-pilot-cf46a28`).**
