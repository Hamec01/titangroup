...
Обновлено: 2026-08-04 19:40 Europe/Helsinki
...
**Следующий шаг**: не выбран явно владельцем — первый вертикальный сценарий ЭТАП 7 теперь полностью
рабочий end-to-end через настоящий UI (не только curl): назначение → открытый период → ввод часов
→ отправка → admin-проверка → `FOREMAN_APPROVED`. Кандидаты: `/admin/periods`/`/admin/review-scopes`
UI (чтобы админ тоже не пользовался только curl); `timesheet.final_approve`; `period.lock`/
`period.export`; прорабский `/api/foreman/*`. Ждать решения владельца о приоритете.
