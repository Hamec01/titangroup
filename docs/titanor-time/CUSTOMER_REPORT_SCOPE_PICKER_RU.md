...
На первом шаге начальник выбирает один из двух равноправных способов:

- **По объектам** — прежний поток «объект → его работники»;
- **По работникам** — прямой поиск по всей рабочей силе за диапазон дат. Объект выбирать не
  требуется. В список входят работники с пересекающим диапазон Employment, назначением или
  canonical-часами. Поэтому работник без объекта остаётся доступен с пометкой «Объект за этот
  период не назначен», а сменивший объект показывает все связанные объекты за диапазон.

В режиме «По работникам» отчёт строится по выбранным людям по всем объектам, на которых у них есть
часы в заданные даты. Это не меняет расчёт или экспорт — в существующий API передаётся прежний
фильтр `employeeIds` (или штатный режим «все»), а `siteIds` не ограничиваются.
...
| `scopeBy` | `workers` — прямой выбор работников; отсутствует → совместимый режим «По объектам» |
