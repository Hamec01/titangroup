...
### 9.9 `FORCE_CLOSE_OPEN_SHIFT` — ADMIN/SUPER_ADMIN только — **`[2026-08-15]` реализовано
(T7A.8B.4A)**
...
BEGIN:
  ...
  3. SELECT EmployeeOpenShift WHERE employeeId FOR UPDATE — **`[T7A.8B.4A]` уточнение реализации**:
     найденная строка обязана дополнительно совпасть по target identity —
     `EmployeeOpenShift.openedByClockEventId === exception.clockEventId` — та же дисциплина, что
     §9.7 уже применяет к своей `EmployeeOpenShift`-ветке (не переизобретена заново). Без этой
     проверки голый `employeeId`-lookup нашёл бы ЛЮБУЮ открытую смену этого работника, включая
     более новую, не связанную с этим исключением. Несовпадение (или отсутствие exception.
     clockEventId вовсе) приравнено к «НЕ НАЙДЕНА» ниже — единый код на оба случая.
     НЕ НАЙДЕНА (буквально, либо через несовпадение identity) -- смена с тех пор закрылась реальным
     Check Out, либо это другая, не связанная смена:
...
**`[2026-08-14] T7A materialization — реализовано.`** §9.2(k), §9.4, §9.5 и внутренний §8.4
catch-up service реализованы в `lib/attendance-materializer.ts`; Check Out/Switch вызывают core
инлайн. Новых permissions/routes/migrations нет. Cron/scheduler wiring остаётся будущим слайсом.
...
Check Out/Switch теперь вызывают `materializeClockShiftCore` (§9.2 шаг k) инлайн в той же
транзакции; при unresolved whole-shift assignment или OPEN overlap смена остаётся `PENDING` для
внутреннего catch-up.

**`[2026-08-15]`** Worker mobile UI (`/worker`) реализован поверх этих же трёх `POST` +
`clock-state` — `app/worker/page.tsx` (server component, session/role/employeeId проверка,
`getClockState`/`listWorkerCurrentAssignments`/`listActionablePeriods` через `Promise.all`) +
`app/worker/WorkerClockPanel.tsx` (client component) + `lib/worker-gps.ts`. Ни одна из четырёх
...
**`[2026-08-14]` T7A.7B Offline Attendance Outbox Client — реализовано.** Новая директория
`lib/offline-outbox/` (browser-only) реализует §6 дословно: `db.ts` (три store —
`clockOutbox`/`localClockState`/`deviceState`, индексы `by-state`/`by-nextAttemptAt`), `sha256.ts`
(синхронная чистая SHA-256 — `payloadHash` включает только что зарезервированный
`deviceSequence`, известный лишь внутри открытой IndexedDB-транзакции, поэтому `await` Web Crypto
там недопустим), `device.ts` (bootstrap/sequence floor/одноразовая rotation при
`DEVICE_NOT_OWNED`+пустой outbox), `outbox.ts` (`atomicEnqueue` — одна readwrite-транзакция на все
три store; `enqueueSwitchSite` резервирует `N`/`N+1` и пишет обе половины группы в той же
транзакции; `applyGroupResult` перечитывает актуальные строки по `groupId`, неполный/смешанный по
категории ответ не трогает ни одну половину), `sync-runner.ts` (`buildBatch` — чистая функция,
...
(`BroadcastChannel`, чисто UX-инвалидация), `projection.ts` (накладывает PENDING/SENDING хвост
поверх последнего `GET /clock-state`). `app/worker/WorkerClockPanel.tsx` переписан: Check In/
Check Out/Switch Site теперь пишут в outbox и синкаются исключительно через уже существующий
`POST /sync` (T7A.7A) — прямые вызовы online-роутов из UI убраны, сами роуты не изменены. Три
реальных бага найдены и исправлены в процессе (hydration-mismatch на `isOnline`, кнопка «Sync now»
игнорировала backoff-гейт, pause обнаруженный mid-sync не отражался в UI-состоянии) — подробности
в `IMPLEMENTATION_STATUS.md`. Проверено в живом браузере (Playwright/Chromium, реальный IndexedDB):
84/84 PASS — online/offline happy path, две вкладки/два независимых device, incomplete/mixed group
response, 503/429/network error backoff, terminal REJECTED, SENDING crash-recovery,
DEVICE_REVOKED/DEVICE_NOT_OWNED/401 сохраняют очередь, первый запуск offline без bootstrap не
создаёт событие, явная post-ACK reconciliation, десктоп+клавиатура+aria-live, best-effort
crash-race на Switch Site (6 итераций, ни одной orphaned-половины). Schema/migrations/permissions/
online route surface не изменены. По-прежнему не реализованы: `GET /attendance/today|week`,
scheduler/auto-submit, exception-review endpoints, admin attendance overview, полноценный service
worker/PWA offline-shell.
...
этими двумя историческими locking-слайсами не затрагивалось; geofence/online clock/materializer
реализованы последующими слайсами, scheduler и exception-review endpoints всё ещё отложены.
...
| 7b | Offline outbox/sync — клиент (T7A.7B) — **`[2026-08-14] реализовано.`** | IndexedDB (§6) — `lib/offline-outbox/` (`db.ts`/`sha256.ts`/`device.ts`/`outbox.ts`/`sync-runner.ts`/`broadcast.ts`/`projection.ts`), `deviceSequence`-счётчик устройства, `WorkerClockPanel`-интеграция (Check In/Out/Switch Site пишут в outbox вместо прямого online-запроса, `runSyncOnce` вызывает уже реализованный `POST /sync`). Проверено в живом браузере (Playwright, реальный IndexedDB): 84/84 PASS — offline check-in→restart→offline check-out→sync, потерянный ответ+повтор, две вкладки, два независимых device, `CLIENT_EVENT_ID_REUSED`/`DEVICE_SEQUENCE_REUSED`-эквивалентные replay/group-сценарии — backend-контракт для всех этих сценариев уже был реализован и протестирован в 7a. |
| 8 | Exception review | **`8a` read foundation — `[2026-08-14] реализовано (T7A.8A)`**: `GET /api/{admin,foreman}/attendance/exceptions[/:exceptionId]` — `lib/attendance-exceptions.ts`, `attendance.exception.read.{assigned,all}` — новая additive DML-миграция `20260816000000_seed_attendance_exception_read_permissions`. Проверено на одноразовом PostgreSQL 16: 83/83 HTTP (permissions/filters/pagination/detail-shapes/foreman-scope/redaction/security). **`8b.1` базовая резолюция — `[2026-08-14] реализовано (T7A.8B.1)`**: `POST /api/{admin,foreman}/attendance/exceptions/:exceptionId/resolve` — только `DISMISS`/`ACKNOWLEDGE_AS_VALID`, `lib/attendance-exception-resolution.ts`, resolver-паттерн §8.5 дословно, `attendance.exception.resolve.{assigned,all}` — новая additive DML-миграция `20260817000000_seed_attendance_exception_base_resolution_permissions`. Проверено на одноразовом PostgreSQL 16: 120/120 (полная матрица §11 по всем 14 типам, FOREMAN mutation scope строже read-scope, реальная многосессионная конкуренция через `pg_stat_activity`). **`8b.2` PAIR_ORPHAN_EVENTS — `[2026-08-14] реализовано (T7A.8B.2)`**: тот же endpoint расширен третьим действием — `pairOrphanEvents` в `lib/attendance-exception-resolution.ts`, §9.8 дословно (INSERT нового `ClockShift(PENDING)` + резолюция named+complementary orphan exceptions + один `AuditEvent`, без новых permissions/миграций). Проверено на одноразовом PostgreSQL 16: 126/126 (применимость/event-валидация/event-reuse/overlap/FOREMAN all-sites union scope/атомарность через реальный FK-violation rollback/реальная многосессионная конкуренция 4 сценария через `pg_stat_activity`). Единственное осознанное расхождение с буквальным §9.8: `403 FOREMAN_SCOPE_INCOMPLETE` вместо дословного `403 FORBIDDEN`, ради консистентности с уже установленным в T7A.8B.1 кодом для той же ситуации — доменный инвариант сохранён. **`8b.3` CONFIRM_SOURCE_ASSIGNMENT — `[2026-08-15] реализовано (T7A.8B.3)`**: тот же admin-endpoint расширен четвёртым действием — `confirmSourceAssignment` в `lib/attendance-exception-resolution.ts`, §9.7 дословно (определение и блокировка одной из трёх целей по заполненности FK исключения — `ClockShiftFragment`/`ClockShift`/`EmployeeOpenShift` с graceful fallback на уже закрывшуюся `ClockShift`, проверка `chosenAssignmentId` по employeeId/siteId/Helsinki calendar-date, `UPDATE` только разрешённого `sourceAssignmentId` NULL→value, RESOLVE exception, один `AuditEvent(CLOCK_SHIFT_ASSIGNMENT_RESOLVED)`, без новых permissions/миграций — те же `resolve.all`/`read.all`, только `ADMIN`/`SUPER_ADMIN`). `FOREMAN`-роут распознаёт action и отвечает `403 FORBIDDEN` до чтения `chosenAssignmentId`/target — структурно недоступно этой роли, не зависит от scope/типа исключения. Материализация не запускается инлайн — `ClockShift.materializationState` не трогается этой транзакцией, следующий catch-up (§8.4) подхватывает нормально; доказано отдельно для всех трёх форм target (fragment создаёт ровно один сегмент, повторный проход идемпотентен). Проверено на одноразовом PostgreSQL 16: 152/152 (permissions/CSRF/body-shape, все 14 типов исключений через матрицу §11, employee/site/date-валидация `chosenAssignmentId` включая Helsinki-полночь и DST-переход 2026-10-25, все три формы target, graceful open→closed fallback, target/exception already-resolved, target-not-found, immutable `ClockEvent` побайтово, DB-триггер отклоняет повторное/произвольное изменение `sourceAssignmentId`, один sanitized `AuditEvent`, regression DISMISS/ACK/PAIR, materializer catch-up+идемпотентность, реальная многосессионная конкуренция (≥2 backend PID через `pg_stat_activity`) для двух одновременных CONFIRM одного исключения и двух разных исключений на одну цель, принудительный rollback через реальный FK-violation после `UPDATE` target+exception но до `COMMIT`). **`8b.4a` FORCE_CLOSE_OPEN_SHIFT — `[2026-08-15] реализовано (T7A.8B.4A)`**: тот же admin-endpoint расширен пятым (и последним ADMIN/SUPER_ADMIN-специфичным) действием — `forceCloseOpenShift` в `lib/attendance-exception-resolution.ts`, §9.9 дословно, с явным уточнением target identity (`EmployeeOpenShift.openedByClockEventId === exception.clockEventId`, та же дисциплина, что уже есть у §9.7 — предотвращает закрытие более новой несвязанной смены). `ADMIN`/`SUPER_ADMIN`-only, `attendance.exception.resolve.all`/`.read.all`, без новых permissions/миграций; `FOREMAN`-роут отвечает `403 FORBIDDEN` на сыром теле, до какой-либо валидации/чтения target. `INSERT ClockShift` (force-closed shape: `checkOutEventId=NULL`, `forceClosed*` все три непустые — `ck_clock_shift_close_mechanism` удовлетворён) → `DELETE EmployeeOpenShift` → `RESOLVE` exception → один `AuditEvent(CLOCK_SHIFT_FORCE_CLOSED)`; ни один `ClockEvent` не создаётся; материализация не запускается инлайн, подхватывается catch-up. `explicitEndAt` — строгий ISO-8601 с обязательным UTC offset (`Z`/`±HH:MM`), date-only и timezone-less отклоняются; строгая хронология `explicitEndAt > openedAt`, без clamp/future-limit. Проверено на одноразовом PostgreSQL 16: 206/206 (дважды подряд, детерминизм конкуренции) — permissions/CSRF/body-shape (включая timezone-less/date-only rejection, `resolutionNote` явно запрещён отдельно от `reason`), 12 неприменимых типов через матрицу §11, terminal exception, originating shift уже закрыта реальным Check Out, несвязанная новая open shift не трогается (и последовательно, и под реальной блокировкой), точное копирование recordedStartAt/site/workArea/sourceAssignmentId (включая nullable), immutable исходный `ClockEvent` побайтово, DB-констрейнт `ck_clock_shift_close_mechanism`, один sanitized `AuditEvent`, replay после успеха → `EXCEPTION_ALREADY_RESOLVED` без дубля, конкуренция A–E (FORCE_CLOSE vs реальный online Check Out — ≥2 backend PID, ровно один canonical `ClockShift`, orphan-checkout существующая семантика; два параллельных FORCE_CLOSE — один `201`/один `409`; несвязанная open shift под реальной блокировкой; FORCE_CLOSE vs CONFIRM_SOURCE_ASSIGNMENT — ни одного lost update; принудительный rollback через реальный FK-violation после `INSERT`+`DELETE` но до `COMMIT`), materializer catch-up (с/без `sourceAssignmentId`, overlap-сценарий — существующий overlap-детектор ловит пересечение, эта задача его не меняла; идемпотентность повторного прохода), полная regression DISMISS/ACKNOWLEDGE_AS_VALID/PAIR_ORPHAN_EVENTS/CONFIRM_SOURCE_ASSIGNMENT (24/24, включая FOREMAN 403-ordering для обоих ADMIN-only действий, CONFIRM vs checkout, CONFIRM `FOR SHARE`-lock T7A.8B.3-follow-up regression). **`8b.4b` REASON_EDIT — `[2026-08-18] реализовано (T7A.8B.4B)`**: см. §9.1a/§10.1-§10.3/§12.4 и
`04_ADMIN_FIRST_API_CONTRACTS.md` §9.1d — с этим действием backend T7A.8B (все шесть resolution-
действий матрицы §11) объявлен завершённым. **`8c.1` Exception Review UI — list + detail —
`[2026-08-18] реализовано (T7A.8C.1)`**: первый UI-слайс поверх T7A.8A/T7A.8B —
...
| 9 | Операционный обзор (T7A.9, roadmap) — **`9a` read foundation `[2026-08-18] реализовано`, `9b` UI `[2026-08-18] реализовано`** | **Владелец, утверждение 2026-08-12**: отдельная сложная страница для conflict/sequence-аномалий в первом пилоте не нужна — вместо неё `ADMIN`/`SUPER_ADMIN` видят **минимальный список/секцию** этих аномалий (`ClockEventIdConflict`, `DeviceEventReceipt(outcome=REJECTED_TERMINAL)`, `FIFO_LEDGER_INCONSISTENT`-класс `AuditEvent`) как часть общего операционного обзора (кто работает сейчас, GPS/sync/missing-checkout exceptions, recorded-vs-reported diff); `FOREMAN` raw conflict payload не получает (`attendance.conflict.read`, §12.1, не выдаётся `FOREMAN`). Это решение закрывает бывший открытый пункт §18.2. **`9a` — `GET /api/{admin,foreman}/overview`** (см. Addendum ниже, `04_ADMIN_FIRST_API_CONTRACTS.md` §9.1e) — `lib/attendance-overview.ts`, 13 operational states, recorded-vs-reported diff, conflicts-секция, bounded query count (n=50/200 → 24 запроса). **`9b` — `/admin` (больше не redirect на `/admin/setup`) и `/foreman`** (`01_SCREEN_MAP.md`) — `lib/attendance-overview.ts`'s новые `getAdminOperationalOverview`/`getForemanOperationalOverview` wrapper'ы (общие для API-роутов и Server Components, без HTTP self-fetch), `components/overview/OverviewView.tsx`. Тест: доступность списка только `ADMIN`/`SUPER_ADMIN`, `403`/access-denied для `FOREMAN`/`WORKER`, санитизация (никогда координаты/raw payload), 139/139 browser-checks на UI-слое. **С `9b` T7A.9 объявляется завершённым целиком.** |
...
matching(url), default-ветка — просто `return;` без единого обращения к caches.* И без единого
собственного вызова `fetch()` — без `event.respondWith(...)` браузер сам выполняет обычную сетевую
обработку запроса, SW-скрипт в этот путь вообще не вмешивается.
```
*(Исправлено T7A.10C.1 FOLLOW-UP, 2026-08-18 — прежняя формулировка ошибочно утверждала, что
default-ветка сама вызывает `fetch(event.request)`; реальный код (`public/sw.js`) везде использует
голый `return;` в этой ветке.)*
