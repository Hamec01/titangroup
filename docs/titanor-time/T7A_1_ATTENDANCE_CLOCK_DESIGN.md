...
| `attendance.exception.resolve.assigned` | `FOREMAN` | только `DISMISS`/`ACKNOWLEDGE_AS_VALID`/`PAIR_ORPHAN_EVENTS` своих объектов — см. v1-рекомендацию §12.4 — **`[2026-08-14] T7A.8B.2 реализовано для DISMISS/ACKNOWLEDGE_AS_VALID/PAIR_ORPHAN_EVENTS`**, `CONFIRM_SOURCE_ASSIGNMENT`/`FORCE_CLOSE_OPEN_SHIFT`/`REASON_EDIT` остаются будущими T7A.8B.3/8B.4 |
| `attendance.exception.resolve.all` | `ADMIN`, `SUPER_ADMIN` | все шесть действий, включая `CONFIRM_SOURCE_ASSIGNMENT`/`FORCE_CLOSE_OPEN_SHIFT` — **`[2026-08-14] T7A.8B.2 реализовано для DISMISS/ACKNOWLEDGE_AS_VALID/PAIR_ORPHAN_EVENTS`**, остальные три остаются будущими T7A.8B.3/8B.4 |
