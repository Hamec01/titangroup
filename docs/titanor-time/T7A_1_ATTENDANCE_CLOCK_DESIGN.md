...
| `attendance.exception.resolve.assigned` | `FOREMAN` | только `DISMISS`/`ACKNOWLEDGE_AS_VALID`/`PAIR_ORPHAN_EVENTS` своих объектов — см. v1-рекомендацию §12.4 — **`[2026-08-14] T7A.8B.1 реализовано только для DISMISS/ACKNOWLEDGE_AS_VALID`**, `PAIR_ORPHAN_EVENTS` остаётся будущим T7A.8B.2 |
| `attendance.exception.resolve.all` | `ADMIN`, `SUPER_ADMIN` | все шесть действий, включая `CONFIRM_SOURCE_ASSIGNMENT`/`FORCE_CLOSE_OPEN_SHIFT` — **`[2026-08-14] T7A.8B.1 реализовано только для DISMISS/ACKNOWLEDGE_AS_VALID`**, остальные четыре остаются будущими T7A.8B.2/8B.3 |
