...
- **квота ПОДТВЕРЖДЕНА владельцем 2026-08-30** (панель Contabo → Object Storage → Storage Settings):
  бакет `Object Storage US-central 3629`, region **United States (Central)**, status Ready,
  **Purchased Space 250 GB**, Used 3.03 MB (0.00 %), Auto-Scaling Off, cancellation date нет.
  Блокер **B02 закрыт**.

**→ Открытый вопрос (не блокер R01, решение владельца до production):** бакет в регионе
**US-central**. DB-dump и uploads содержат персональные данные работников (имена, email, контакты)
и GPS-координаты. Хранение таких бэкапов в США — трансграничная передача персональных данных за
пределы ЕЭЗ (GDPR). Contabo имеет и EU-регионы. Владельцу до R14 решить: (а) принять US-хранилище
как есть, оформив это в политике обработки персональных данных, либо (б) завести бакет в EU-регионе
и переключить `TT_MIRROR_ROOT` на него. Механически R01 от этого не зависит — off-box mirror
работает; при выборе (б) меняется только один путь в конфиге.
