...
| 6 | Чёрный текст в попапах карты (были почти невидимы на тёмной теме) | ✅ ГОТОВО. Коммит `5ec2c4b`. CSS-правка `.maplibregl-popup-content{color:#1a1a1a}` в `globals.css`. Пилот → `t97-pilot-5ec2c4b`. На пилоте уже виден реальный `ShiftPresenceSample` (владелец снял точку из приложения в 15:37, «в зоне», ±18.7 м) — 2b работает end-to-end. |
