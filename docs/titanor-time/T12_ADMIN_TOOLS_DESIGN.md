...
| 7 | «Отклонить» на проблеме учёта читалось как «отклонить часы» | ✅ ГОТОВО. Коммит `32cd488`. Кнопка → «Снять сигнал» / «Clear alert», статус → «Снято» / «Cleared», пояснение переписано («часы, смена и отметки НЕ меняются; если часы неправильные — «Изменить часы» или вернуть работнику»). |
| 8 | Работника допрашивают при правке своего табеля до отправки | ✅ ГОТОВО. Коммит `b86824e`. Убрано требование «причина изменения зафиксированного времени» — работник правит и отправляет, аудит-запись `ClockShiftAdjustment` пишется с дефолтной причиной «Изменено работником». Сырой `clockAdjustmentReasons.<uuid>: required…` больше не утекает. Плашка «причину не удалось загрузить, обратитесь к администратору» → спокойное «Табель снова открыт для правок…». |
| 9 | Нет кнопки «Утвердить» — блокирует «Продолжить исправление»; в черновике правки пропала пятница | ✅ ГОТОВО. Коммит `b86824e`. На карточке табеля с открытой правкой — кнопка «Отменить правку и вернуться к утверждению». `openCorrectionDraft` пере-сеет черновик с ТЕКУЩЕЙ версии при повторном открытии, если версия ушла вперёд (новое событие аудита `CORRECTION_DRAFT_RESEEDED`) — карточка и редактор больше не расходятся. |

| 10 | «Пропавшая пятница» + модель «неделя работника до понедельника» | ✅ ГОТОВО. Коммиты `79737fa`, `2232fa8`. См. ниже. |
| 11 | В карточке уведомления — дата и время, когда работник сдал/внёс правки (маленьким шрифтом сразу под «ПРЕДУПРЕЖДЕНИЕ») | ✅ ГОТОВО. `AdminNotificationView.eventAt` = `timesheet.currentVersion.createdAt` (момент заморозки последней версии), для допусков — `null` (там нет действия). `NotificationCenter` рендерит `formatEventAt` в drawer-карточке и в тосте. `_test-timesheet-approval-notifications` 14/14. Кода схемы не трогали, миграций нет. |

**Все → на пилоте `t97-pilot-126f527` (2026-08-28).** Бэкапы перед каждым деплоем в
`/home/deploy/backups/titanor-time-pilot/`. На пилоте теперь есть контейнер
`t97-pilot-scheduler` (авто-отправка в дедлайн + генерация периодов + ретенция).

## §10 — модель редактирования табеля («неделя работника до понедельника 23:59»)

**«Пропавшая пятница».** Табель Andrei Sakki за 24–30 авг **корректен** (версия 2, пятница 7ч,
FINAL_APPROVED). Экран с пятницей «—» — это **старая отменённая правка** (`f5c8f396`, на базе
версии 1, созданная до того, как прилетел поздний чек-аут за пятницу). Причины и фиксы:
- `autoCloseOpenCorrectionsForTimesheet` — открытые правки админа (`PENDING`/`DRAFT_OPEN`)
  авто-закрываются (`CORRECTION_AUTO_CLOSED`), когда табель уходит из-под них: поздняя
  синхронизация чек-аута (materializer), возврат работнику, переоткрытие работником.
- `applyInReviewCorrection` отказывает, если `basedOnVersionId` ≠ текущей версии (никогда не
  затирает живую версию устаревшим снимком).
- Страница `/admin/corrections/[id]` для `REJECTED`/`APPROVED` показывает «эта правка
  отменена/применена — возможно, на устаревшей версии» + ссылку на живой табель, без старой
  таблицы дней.

**Модель (подтверждена владельцем).** Дедлайн = конец цикла + 1 день, 23:59 Хельсинки
(`CompanyAttendancePolicy.cutoffDaysAfterPeriodEnd = 1`, миграция `20260828160000`). Недельный
цикл Пн–Вс → следующий понедельник 23:59 (8 дней на правки); двухнедельный → день 15.
- До дедлайна: работник **свободно правит** (с чек-ин/аут или без), «Отправить» — мягкий
  обратимый сигнал. Кнопка **«Внести правки»** переоткрывает табель (`reopenWorkerTimesheetForEdits`:
  `SUBMITTED`/`FOREMAN_APPROVED`/`FINAL_APPROVED` → `DRAFT`, черновик из текущей версии,
  `systemReopenGeneration` НЕ трогается — дедлайн остаётся границей периода). Перебивает раннее
  утверждение админом.
- В дедлайн: авто-отправка (`attendance-auto-submit`, `t97-pilot-scheduler`), блокировка.
  Работник видит «Неделя закрыта и передана руководителю».
- Работник в списке часов видит `учтён −30 мин обед (не оплачивается)` — те же цифры, что у
  админа и в отчётах.

Тесты: `_test-worker-reopen-edit-window` 14/14, `_test-admin-direct-edit` 31/31 (авто-закрытие
правки при возврате), корр./обеды/утверждение регрессии — зелёные.

## Логическая цепочка (модель владельца, 2026-08-28)

1. **Админ** создаёт работника, объект, шаблон табеля, время.
2. **Работник** в приложении видит свои объекты → чек-ин/чек-аут в заданное время → **свободно
   редактирует неделю до отправки** (без вопросов «почему») → отправляет админу/прорабу. Забыл —
   отправится автоматически.
3. **Админ** видит приход/уход, часы, карту, кто работал — полную картину. **Может изменить часы**
   («Изменить часы» — без причины, работник не уведомляется).
4. **Админ** получает табель по утверждённому времени → смотрит, при необходимости правит →
   **утверждает**. После утверждения работник изменить ничего не может. Табель остаётся в базе
   (неизменяемая версия).
5. **Админ** делает экспорт. Может править и после утверждения — но каждая правка в истории
   (`AuditEvent` + корректировка отдельной версией). Это уже финал.

Принципы: работник — ноль трения; админ — полная видимость и право правки, но всё в аудите;
...
### §10 добивка (2026-08-28, `t97-pilot-126f527`)

- **Баг «Внести правки» на утверждённом табеле** → `reinitializeDraftFromVersion` теперь
  идемпотентна (чистит дни + planned shifts черновика перед пере-заполнением). Черновик нёс
  planned shifts от прошлого reinitialize → `Unique constraint (draftId,date,sourceAssignmentId)`.
  Тест `_test-worker-reopen-edit-window` 18/18 (+ кейс FINAL_APPROVED с остаточными planned shifts).
- Уведомление: **«внёс правки в табель за неделю …»** если текущая версия > 1 (переотправка после
  ранней сдачи), иначе «сдал табель». `AdminNotificationView.timesheetIsRevision`.
- В колокольчике у каждого уведомления явная кнопка **«Убрать» / «Dismiss»** рядом с «Открыть
  табель» (крестик «×» тоже остался).
