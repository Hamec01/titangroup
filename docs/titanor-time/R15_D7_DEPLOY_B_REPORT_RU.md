...
Ветка `feature/titanor-time-foundation`, коммит **`40778bb`**. Разрешение владельца на разработку —
2026-09-03. Образ `titanor-time-app:d7b-b23bdd8` (= `d7b-40778bb`, собран из этого дерева; в образе
кода теста/отчёта нет). Перед production будет свежая пересборка под финальный SHA.
...
## 5. Disposable-проверка (образ `d7b-b23bdd8`, чистый PG16)
...
Web-only swap `d7d3-5690632` → `d7b-b23bdd8` (**без миграции**, схема остаётся 100). Standard
