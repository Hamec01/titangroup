...
**Статус:** образ `titanor-time-app:d7f-d216482` развёрнут на production 2026-09-03 ~19:34 UTC
(web-only swap, без миграции, схема 100, простой ≈ 2.6 c — со 2-й попытки, см. §3.1).
**Владелец подтвердил (2026-09-04): Deploy F можно считать технически live.** Production-отчёт —
**`R15_D7_DEPLOY_F_PROD_REPORT_RU.md`**. **Полный R15 owner sign-off НЕ дан** — открыты F02
(реальный Android), F04 (backup публичного сайта), процесс разбора исключений начальником.
Запись «Что нового» `/guide` + правка F03 — не в работающем образе, поедут следующим деплоем.
**Production без явного разрешения владельца не обновлять.** Этот документ — дев/disposable-отчёт
(§1–§3, §6 = хроника развёртывания).
