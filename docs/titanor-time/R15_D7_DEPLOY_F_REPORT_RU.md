...
**Статус (указание владельца, 2026-09-04):** образ `titanor-time-app:d7f-d216482` **физически
развёрнут** на production 2026-09-03 ~19:34 UTC (web-only swap, отдельно разрешён владельцем;
без миграции, схема 100, простой ≈ 2.6 c — со 2-й попытки, см. §3.1). **Но Deploy F и запись
«Что нового» в `/guide` НЕ считаются production-live** до отдельного production deploy report
и его ревью владельцем — см. **`R15_D7_DEPLOY_F_PROD_REPORT_RU.md`**. **Production без явного
разрешения владельца не обновлять.** Этот документ — дев/disposable-отчёт (§1–§3, §6 = хроника
развёртывания). Deploy F закрывает R15-D7 (A→F) технически; полный R15 sign-off не дан.
