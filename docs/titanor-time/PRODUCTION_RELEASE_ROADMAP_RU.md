...
| ~~B08~~ | In-memory rate limit, доверие первому `X-Forwarded-For`, слабые public admin/contact/upload controls | Обход ограничений и недостаточный security boundary | ✅ **ЗАКРЫТ 2026-08-30.** Titanor Time: **R07-A (`8f3795f`) на пилоте** (`t97-pilot-8724480`, миграция 97) — rate-limit DB-backed shared (`RateLimitCounter`), client IP trusted-proxy-aware (`lib/client-ip.ts`, rightmost-minus-hops, `TITANOR_TRUSTED_PROXY_HOPS`, поддельный leftmost XFF не доверяется). Public site: **R07-B DEPLOYED + PASS** (`titanorgroup-web:site-3321c09`) — admin login rate-limit + timing-safe пароль + CSRF + журнал входов; contact rate-limit + SMTP timeouts + очищенный лог; uploads magic-bytes + GIF-отказ + sharp re-encode + path-traversal; security-заголовки + `poweredByHeader:false`. Отчёт `R07B_PUBLIC_SITE_HARDENING_REPORT_RU.md`. R11: Caddy `trusted_proxies_strict` + `TITANOR_TRUSTED_PROXY_HOPS=2`. |
| ~~B09~~ | Нет долговременного зашифрованного GPS-архива до удаления raw records | Возможна необратимая потеря истории координат | ✅ **ЗАКРЫТ 2026-08-30 — R08 DEPLOYED + PASS** (пилот `t97-pilot-6a47ed3`, DB 98): миграция 98 `GpsArchiveDay` ledger; ежедневный AES-256-GCM+gzip архив per reading-day на off-box `/mnt/250gb` (host-скрипт + systemd `titanor-time-gps-archive@pilot.timer` `enabled`, как R01 backup); retention удаляет raw только за полностью-VERIFIED дни; нет/битый `GPS_ARCHIVE_ENCRYPTION_KEY` → retention не удаляет ничего. Первый прогон: 5 дней → 5 VERIFIED, 0 FAILED. 5 test-файлов / ~100 assert + disposable-verify 17/0. Отчёт `R08_GPS_ARCHIVE_REPORT_RU.md`. Открыто (не блокер): worker-notice + PII-политика — утверждает ответственное лицо Titanor; R08.1 (читаемый экспорт). |
| ~~B10~~ | Mobile overflow в ADMIN и части WORKER-экранов; неполные UX-потоки | Рабочие сценарии на телефоне неудобны или частично недоступны | ✅ **ЗАКРЫТ 2026-08-30 — R09** (ADMIN + WORKER): `html,body{overflow-x:hidden}`, 12 admin-таблиц в `.worker-table-scroll`, шапка `/admin` переносится; browser-чек-лист по 4 viewport `R09_BROWSER_ACCEPTANCE_RU.md`. FOREMAN-overflow — вне объёма R09 (backlog). |
...
| R09 | WORKER/FOREMAN/ADMIN UX | R03, R05, R07 | Нет | **Код готов, CI зелёный, ждёт pilot-деплоя 2026-08-30** (`d5a8d24`…`fe33e31` + R09.11). Объём сужен владельцем: **FOREMAN UI полностью исключён** (backlog), R09.9 — backlog. Ни одной миграции. R09.1 `/admin/users` поиск+фильтры+пагинация; R09.2 человеческие причины запрета (13 admin-страниц, `AccessDeniedNotice`); R09.5 «истекающие документы» карточка в task-center; R09.6 overflow sweep ADMIN+WORKER (**B10**); R09.7 `WorkerClockPanel` split 1160→951 (ноль изменений поведения); R09.8 `WorkerCardNav` — навигация по 4 раздельным страницам карточки работника; R09.10 no-op (не тронуто ни одного `route.ts`); R09.11 отчёт + browser-чек-лист. 5 новых test-файлов. Отчёт `R09_UX_REPORT_RU.md`. **Deploy:** один pilot image-swap (без миграции) — по подтверждению владельца. |
...
**Статус:**
- **R07-A** (Titanor Time) — DONE + **РАЗВЁРНУТ на пилот 2026-08-30, PASS**. `899862c`
  ...
  выполнен (70 negative-проверок). CI 6/6 (`139221d`, `0b9d1f2`). Отчёт
  `R07A_SECURITY_HARDENING_REPORT_RU.md`.
  - **Образ `t97-pilot-8724480`** (`sha256:4516b393c686…`, 792 MB, из HEAD `8724480`).
  - Disposable-env verify (до деплоя) PASS: from-zero 97 + **restored-pilot dump 96→97** (1 миграция,
    0 failed); app/scheduler healthy на 97; security headers + нет `X-Powered-By`; login/logout/
    recovery smoke; **rate-limit B08 — 429 + строка по rightmost X-Forwarded-For (не поддельный
    leftmost) + пережил `docker restart`**; malformed UUID → 404 не 500; prod baseline не изменён.
  - **Deploy 2026-08-30** (`deploy-8724480.sh`, владелец): backup on-box + off-box mirror,
    `migrate deploy` 96→97 (0 failed, `RateLimitCounter` создана, **probe-строки rate-limit удалены,
    0 строк**), swap app+scheduler на `8724480` (оба `healthy`), scheduler lease у нового holder +
    renew, все 4 операции, security headers на пилоте, prod baseline (`daa2edbb`) не изменён.
    Rollback-контейнеры `-pre-8724480` (на `t97-pilot-256565a`) сохранены; scheduler-pre exited 0
    (штатный graceful — без stale lease). **Пилот: `t97-pilot-8724480`, DB 97/97.**
...
- **R07-B** (публичный сайт) — **КОД DONE 2026-08-30** (`f8dd3f5`/`02a9c9b`/`6644254`/`5e0d7f3`/
  `5a29496`/`6ba93c4`), отчёт `R07B_PUBLIC_SITE_HARDENING_REPORT_RU.md`:
  - §16.2 admin: login rate-limit 10/15мин (in-memory fixed-window, trusted client-IP); пароль
    HMAC-SHA256 + `timingSafeEqual`; CSRF `X-Requested-With: titanor-admin` на login/logout/images/
    vacancies/service-content; cookie `HttpOnly`+`SameSite=Strict`+`Secure`; журнал входов
    `lib/admin-audit.ts` → append-only файл (`ADMIN_AUDIT_LOG`), без пароля/токена.
  - §16.3 contact: rate-limit 5/15мин; SMTP timeouts 10/10/20s; лог сбоя `[code] message`, без
    тела формы; honeypot+escape сохранены; битое тело → 400.
  - §16.4 uploads: формат по magic-bytes (не Content-Type); JPEG/PNG/WebP, GIF — отказ; размер до
    обработки; sharp re-encode (strip EXIF/GPS/ICC, downscale > 4096); `Content-Disposition:
    inline` + nosniff; path-traversal — allowlist + resolve-containment.
  - Заголовки: `poweredByHeader:false` + `/:path*` (nosniff, X-Frame `SAMEORIGIN`, Referrer-Policy,
    COOP, Permissions-Policy lockdown, HSTS 2y); нет `X-Robots-Tag` (сайт индексируется);
    `robots Disallow` `/ship-admin-portal` `/api/` `/uploads/`; удалены статические
  ...
  - **Slice 7 — DEPLOYED + PASS 2026-08-30.** `ops/site/deploy-site-r07b.sh` (immutable-swap
    `titanorgroup-web:site-3321c09`; smoke-тест на `titanorgroup-web-verify:3199` ПЕРЕД подменой
    live; backup обоих томов on-box+off-box; авто-rollback; Titanor Time prod+pilot baseline
    перепроверён — не изменён). 3 прогона владельца: первые 2 безопасно abort на pre-swap gates
    (live не тронут), 3-й прошёл. 3 бага (все «gate падает молча вместо `die`»): `d60a125`
    `http_code`→`000` fail-closed; `2999397` repo sanity для linked worktree (`.git`—файл);
    `3321c09` env-check без pipeline + mount-check `/mnt/250gb` + `SWAP_STARTED` после `rename`.
    `--self-test` покрывает оба класса, в CI. Владелец подтвердил доставку contact-письма
    (`projects@titanorgroup.fi`). **Rollback-контейнер `titanorgroup-web-1-pre-r07b` сохранён.**
    Compose-детач задокументирован (не `docker compose up -d` в `/home/deploy/projects/titanorgroup`).
  - **UX backlog (не чинить сейчас):** `mailto:` корректны, но у владельца в Chrome/Windows нет
    внешнего mailto-handler → окно письма не открывается. Позже: «скопировать email» / переход к
    встроенной форме. Плюс `/fi` `<html lang="en">` (i18n-этап).
...
**Статус: DEPLOYED + PASS 2026-08-30.** Отчёт `R08_GPS_ARCHIVE_REPORT_RU.md`. Пилот
`titanor-time-app:t97-pilot-6a47ed3`, DB 98/98, app+scheduler `healthy` restarts 0.
`deploy-6a47ed3.sh` (владелец): backup on+off-box, migrate 97→98 (0 failed, `GpsArchiveDay`+trigger),
swap, verify (`.runtime/gps-archive.cjs` bogus→2/empty-key→3, retention `retentionOutcome:ok`, R07-A
регрессия, prod baseline). Rollback-контейнеры `-pre-6a47ed3` (на `t97-pilot-8724480`) — сохранены.
Disposable-verify до деплоя: **17/0** (migrate 98, `/api/ready` current, write→off-box→promote,
`.enc` расшифровывается в seeded-строки с точными координатами, retention удаляет ровно 3
архивированных >90д строки, keyless→`skipped_no_archive_key`, `t97-pilot-8724480` толерантен к
схеме 98). Первый ручной прогон архива + `systemctl enable --now titanor-time-gps-archive@pilot.timer`
(владелец): 5 sealable дней (2026-08-24…28) → 5 written → off-box sync 5/5 → **5 VERIFIED, 0 FAILED**;
scheduler retention `retentionOutcome:ok retentionDeleted:0`. Commits `9bcf16f` (миграция 98 `GpsArchiveDay`) · `19544cc`
...
(`.runtime/gps-archive.cjs` write/promote) · `7253bf9` (`gps-archive-titanor-time.sh` + systemd
`titanor-time-gps-archive@`, 05:10 UTC, staging→`/mnt/250gb` c SHA-256 verify) · `64070fc`
...
- **Owner action items** — `R08_GPS_ARCHIVE_REPORT_RU.md` §6: ✅ ключ · ✅ образ + disposable-verify ·
  ✅ deploy-скрипт запущен · ✅ systemd-таймер `enabled` + первый прогон. **Открыто (не блокер,
  внутреннее приложение фирмы):** текст уведомления работникам + политика перс. данных —
  утверждает владелец бизнеса / ответственное лицо Titanor.
- **Остаток:** R08.1 — читаемый TXT/CSV экспорт из архива по запросу (ТЗ §9.4), отдельная задача.
- **Rollback-контейнеры `-pre-6a47ed3` сохранены** до отдельного разрешения владельца.

---

### R09. UX WORKER, FOREMAN, ADMIN/SUPER_ADMIN

**Статус 2026-08-30: код готов, CI зелёный, ждёт одного pilot image-swap (без миграции) по
подтверждению владельца.** Отчёт `R09_UX_REPORT_RU.md`, план+объём `R09_UX_AUDIT_AND_PLAN_RU.md`,
browser-чек-лист `R09_BROWSER_ACCEPTANCE_RU.md`. **Объём сужен владельцем 2026-08-30: весь FOREMAN
UI исключён** (mobile-nav, bulk-подтверждение, task-list-приоритеты прораба → backlog после
production); split крупных admin-модулей (R09.9) → backlog; объединение worker card в один экран →
backlog (R09.8 = только навигация между 4 раздельными страницами). Сделано: `d5a8d24` (R09.1
`/admin/users` поиск/фильтры/страницы) · `f0e081f` (R09.2 человеческие причины запрета, 13
admin-страниц) · `b6f052c` (R09.5 «истекающие документы» в task-center) · `6099e6b` (R09.6 overflow
sweep ADMIN+WORKER) · `7184aaa` (R09.7 `WorkerClockPanel` split, ноль изменений поведения) ·
`fe33e31` (R09.8 `WorkerCardNav`) · R09.11 (этот отчёт). R09.10 (`guardApiRequest` on-touch) —
no-op: ни один `app/**/route.ts` не изменён.
