...
| B08 | In-memory rate limit, доверие первому `X-Forwarded-For`, слабые public admin/contact/upload controls | Обход ограничений и недостаточный security boundary | 🟡 **R07-A (`8f3795f`) DONE + на пилоте (`t97-pilot-8724480`, миграция 97)**: rate-limit — DB-backed shared (`RateLimitCounter`), client IP — trusted-proxy-aware (`lib/client-ip.ts`, rightmost-minus-hops, `TITANOR_TRUSTED_PROXY_HOPS`, поддельный leftmost XFF не доверяется). Public admin/contact/upload controls — **R07-B КОД DONE (`6ba93c4`)**: admin login rate-limit + timing-safe пароль + CSRF + журнал входов; contact rate-limit + SMTP timeouts + очищенный лог; uploads magic-bytes + GIF-отказ + sharp re-encode + path-traversal; security-заголовки + `poweredByHeader:false`. 7 test-файлов / 133 assert, CI-джоба. **На live-сайт не развёрнут** (Slice 7 отдельно). Отчёт `R07B_PUBLIC_SITE_HARDENING_REPORT_RU.md`. |
...
| R07 | Security hardening приложений/API | R02, R04, R05 | Нет | **R07-A DONE + РАЗВЁРНУТ на пилот 2026-08-30 (PASS)** — `t97-pilot-8724480`, DB 97/97, app+scheduler `healthy`. (`899862c`/`8f3795f`/`c413748`/`ecfb302`): (A) security headers + noindex + safe error/not-found; (B) trusted-proxy client IP `TITANOR_TRUSTED_PROXY_HOPS` + DB-backed shared rate limiter (`RateLimitCounter`, миграция 97) — **B08**; (C) `requireUuidParam` до Prisma на 26 маршрутах — **B11**; (D) `guardApiRequest` helper + `/api/auth/{session,logout,logout-all}`. Log audit — уже соответствует. PASS-критерий (70 negative-tests) + deploy-verify (security headers, rate-limit-probe cleanup, scheduler, backup on+off-box, prod unchanged). Отчёт `R07A_SECURITY_HARDENING_REPORT_RU.md`. **R07-B (публичный сайт) — КОД DONE (`6ba93c4`), все проверки зелёные, на live-сайт не развёрнут** (Slice 7 отдельно): admin login rate-limit + timing-safe пароль + CSRF + журнал входов; contact rate-limit + SMTP timeouts + очищенный лог; uploads magic-bytes + GIF-отказ + sharp re-encode + path-traversal; security-заголовки + `poweredByHeader:false` + `robots Disallow`. 7 test-файлов / 133 assert, CI-джоба `public-site-quality`. Отчёт `R07B_PUBLIC_SITE_HARDENING_REPORT_RU.md`. **Остаток:** R07-A.1 (миграция ~130 маршрутов на guard, пошагово с ревью, с R09); R07-B Slice 7 (deploy live-сайта, ждёт решений владельца). |
