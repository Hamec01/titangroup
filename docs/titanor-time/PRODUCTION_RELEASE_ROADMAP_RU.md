...
- **Owner action items** — `R08_GPS_ARCHIVE_REPORT_RU.md` §6: ✅ ключ · ✅ образ + disposable-verify ·
  ✅ deploy-скрипт запущен · ✅ systemd-таймер `enabled` + первый прогон. **Открыто:** текст
  уведомления работникам + политика перс. данных (владелец/юрист).
