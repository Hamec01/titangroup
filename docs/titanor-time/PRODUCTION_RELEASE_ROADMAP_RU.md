...
| B09 | Нет долговременного зашифрованного GPS-архива до удаления raw records | Возможна необратимая потеря истории координат | 🟡 **R08 КОД DONE (`506321e`)**: миграция 98 `GpsArchiveDay` ledger; ежедневный AES-256-GCM+gzip архив per reading-day на off-box `/mnt/250gb` (host-скрипт + systemd, как R01 backup); retention удаляет raw только за полностью-VERIFIED дни; нет/битый `GPS_ARCHIVE_ENCRYPTION_KEY` → retention не удаляет ничего. 5 test-файлов / ~100 assert, e2e достаёт координаты удалённых строк из off-box `.enc`. **На пилот не развёрнут** (нужен ключ + systemd). Отчёт `R08_GPS_ARCHIVE_REPORT_RU.md`. |
