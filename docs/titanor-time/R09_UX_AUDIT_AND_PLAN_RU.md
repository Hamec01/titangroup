...
| **R09.6** | Overflow-sweep **WORKER + ADMIN** (foreman не трогаем): CSS-правки (`min-width:0`, `overflow-x:auto` на таблицах/код-блоках, `overflow-wrap`), без изменения разметки/поведения. | низкий | CSS-lint; browser-чек-лист по 4 viewport |
| **R09.7** | `WorkerClockPanel` split: извлечь под-компоненты — **чистая экстракция, ноль изменений поведения**. Никаких новых фич/правок логики. | средний | регрессия: рендер-тест ключевых affordance; существующие worker-тесты зелёные; browser-чек-лист clock |
| **R09.8** | Карточка работника — **только навести порядок** (без объединения): `/[employeeId]`, `/profile`, `/timeline`, `/locations` остаются отдельными страницами; добавить понятную навигацию между ними, унифицировать заголовки / breadcrumbs / быстрые действия, убрать устаревшие/битые ссылки. Глубокие изменения карточки → backlog после production. | низкий | typecheck/build; все под-данные на месте; browser-чек-лист |
| **R09.10** | `guardApiRequest` rollout **только по маршрутам, реально изменённым в R09.1–R09.8** (users, overview, worker-card sub-routes) — по одному, с ревью; envelope байт-идентичен. Blind codemod запрещён. | средний | `_test-api-guard` расширить; per-route negative-тесты |
...
CI), по отдельному подтверждению владельца. Промежуточных деплоев нет. Production/DNS/Caddy/
Cloudflare не трогать.

---

## 4. Подтверждённый объём (владелец, 2026-08-30)

**В R09:** R09.1 · R09.2 · R09.5 · R09.6 · R09.7 · R09.8 · R09.10 · R09.11.

**Исключено / перенесено (не трогаем сейчас):**
- **Весь FOREMAN UI** — заказчик это не обговаривал → R09.3 (adaptive nav), R09.4 (bulk-
  подтверждение) и любые правки прораба **не входят в R09**.
- **R09.9** (split крупных admin-модулей `ExceptionActionPanel` / `AdminWorkerProfileForm` /
  `PolicyForm`) — только если начинает раздувать этап → **не делаем**, в backlog.
- Объединение карточки работника в один табовый экран → backlog после production.

**Решения по деталям:**
- **R09.8** — не объединять, только навигация/заголовки/breadcrumbs/быстрые действия + убрать
  устаревшие ссылки.
- **R09.2** — короткий человеческий текст под конкретный раздел/действие, RU/EN через существующий
  i18n; permission-код НЕ показывать обычному пользователю (можно оставить в `title`/`aria`/debug
  metadata для поддержки); без длинных объяснений и жаргона.
- **R09.7** — только экстракция, ноль изменений поведения.
- **R09.10** — только реально трогаемые маршруты.

---

## 5. Выполнение (2026-08-30)

Все слайсы сделаны. Итоговый отчёт — `R09_UX_REPORT_RU.md`, browser-чек-лист —
