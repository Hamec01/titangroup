#!/usr/bin/env bash
# Titanor Time — pilot deploy of R07-A (security hardening). Commit 8724480.
# Same fail-closed / auto-rollback / stale-lease structure as ops/titanor-time/deploy-pilot-256565a.sh
# (R06-B.1). Canonical copy in the repo; identical copy at /home/deploy/app-data/t97-pilot/deploy-8724480.sh.
#
# This deploy DOES change the schema: one additive migration
#   20260830140000_add_rate_limit_counter   (CREATE TABLE "RateLimitCounter")   96 -> 97
# and swaps the image
#   titanor-time-app:t97-pilot-256565a  ->  titanor-time-app:t97-pilot-8724480
#
# R07-A behaviour that goes live:
#   * security response headers on every route + no X-Powered-By + robots/noindex;
#   * client IP is trusted-proxy-aware (TITANOR_TRUSTED_PROXY_HOPS, default 1 = Caddy) — the
#     leftmost X-Forwarded-For value is never trusted;
#   * rate limiting is DB-backed (RateLimitCounter) — shared + survives a restart;
#   * a malformed [id] path param returns 404, never Prisma P2023 / HTTP 500;
#   * guardApiRequest on /api/auth/{session,logout,logout-all}.
#
#   Run:  bash /home/deploy/app-data/t97-pilot/deploy-8724480.sh
#
set -Eeuo pipefail

IMAGE=titanor-time-app:t97-pilot-8724480
...
APP_PORT=3297
PREV_TAG=t97-pilot-256565a          # image to roll back to
MARK=8724480                        # rollback-container suffix ("state before deploying <MARK>")
EXPECT_MIGRATIONS=97
PREV_MIGRATIONS=96
...
rollback() {
  ...
  echo "  NOTE: migration 97 (RateLimitCounter) stays applied — it is additive and the"
  echo "  $PREV_TAG image tolerates an extra table ('schema: ahead' -> still 200). No down-migration."
...
echo "  ok — pilot DB at $PRE migrations"
...
echo
echo "== 5/8  migrate t97-pilot-db 96 -> 97 (fail-closed) =="
...
[ "$(psqlq "SELECT to_regclass('public.\"RateLimitCounter\"') IS NOT NULL")" = "t" ] || die "RateLimitCounter table not created"
LAST_MIG=$(psqlq "SELECT migration_name FROM _prisma_migrations ORDER BY finished_at DESC NULLS LAST LIMIT 1")
echo "  migrations: $PRE -> $APPLIED (0 failed); newest = $LAST_MIG; RateLimitCounter present"
...
echo "  -- R07-A: security headers + no X-Powered-By + robots --"
H=$(curl -s -D - -o /dev/null --max-time 10 "$BASE/login" || true)
for want in "X-Content-Type-Options: nosniff" "X-Frame-Options: DENY" "Referrer-Policy: strict-origin-when-cross-origin" "Cross-Origin-Opener-Policy: same-origin" "Permissions-Policy: geolocation=(self)" "Strict-Transport-Security: max-age=63072000" "X-Robots-Tag: noindex"; do
...
case "$UC" in 5*|000) note_fail "malformed UUID route returned $UC";; *) echo "     ok (4xx — full 404 path covered by CI _test-malformed-uuid + disposable verify)";; esac
...
echo
echo "Post-deploy manual check (recommended): open the pilot UI, log in, and confirm normal use."
echo
echo "Manual rollback later (back to $PREV_TAG; migration 97 stays — additive, tolerated):"
