#!/usr/bin/env bash
# Titanor Time — pilot deploy of R09 (WORKER + ADMIN UX before production). Commit edd950c.
# Same fail-closed / auto-rollback / stale-lease structure as ops/titanor-time/deploy-pilot-6a47ed3.sh
# (R08), deploy-pilot-8724480.sh (R07-A), deploy-pilot-256565a.sh (R06-B.1). Canonical copy in the
# repo; identical copy at /home/deploy/app-data/t97-pilot/deploy-edd950c.sh.
#
# This deploy does NOT change the schema. R09 is pure UI / navigation / i18n + one client-component
# extraction — no migration. It is a straight image swap:
#   titanor-time-app:t97-pilot-6a47ed3  ->  titanor-time-app:t97-pilot-edd950c
# The DB stays at 98 migrations; `migrate status` is checked read-only (never `migrate deploy`).
#
# R09 behaviour that goes live (all UI, no business-logic change):
#   * /admin/users  — search (username OR email) + role/status filters + prev/next paging
#   * 13 admin pages — human "no access" text instead of a raw permission code (code kept in
#     title / data-permission for support)
#   * /admin — a "documents needing attention" card in the task-center (expired / expiring quals)
#   * ADMIN + WORKER — no horizontal overflow: wide tables scroll inside their own container
#   * /worker clock screen — WorkerClockPanel split into app/worker/clock-panel/* (SAME behaviour)
#   * /admin/workers/[id] (+ /profile /timeline /locations) — one shared "Workers > Name" breadcrumb
#     + a link row between the four pages (they stay separate)
# R07-A (security headers, DB-backed rate limit, guardApiRequest, malformed-UUID 4xx) and R08
# (gps-archive bundle + fail-closed retention) are re-verified below to prove nothing regressed.
#
#   Run:  bash /home/deploy/app-data/t97-pilot/deploy-edd950c.sh
#
set -Eeuo pipefail

IMAGE=titanor-time-app:t97-pilot-edd950c
...
APP_PORT=3297
PREV_TAG=t97-pilot-6a47ed3          # image to roll back to
MARK=edd950c                        # rollback-container suffix ("state before deploying <MARK>")
EXPECT_MIGRATIONS=98                 # unchanged — R09 adds no migration
...
rollback() {
  ...
  echo "  No schema change to undo — R09 added no migration. Straight image swap back."
...
# ============================================================================================
echo "== 0/7  concurrency + state guard =="
...
[ "$app_img" = "titanor-time-app:$PREV_TAG" ] || die "t97-pilot-app is on $app_img, expected titanor-time-app:$PREV_TAG"
[ "$sch_img" = "titanor-time-app:$PREV_TAG" ] || die "t97-pilot-scheduler is on $sch_img, expected titanor-time-app:$PREV_TAG"
echo "  ok — app+scheduler on titanor-time-app:$PREV_TAG, no -pre-$MARK containers"

echo
echo "== 1/7  candidate image present + labelled =="
...
# R09 ships app/worker/clock-panel/* — the Next standalone bundle must carry the new chunk. A cheap
# proof the right tree was built: the module exists in the image's app source copy.
docker run --rm -w /app --entrypoint sh "$IMAGE" -c 'test -f .runtime/gps-archive.cjs' \
  || die ".runtime/gps-archive.cjs missing from the image — R08 runtime bundle regressed"
...
grep -qE '^GPS_ARCHIVE_ENCRYPTION_KEY=[A-Za-z0-9+/]{42,}={0,2}$' "$ENVFILE" \
  || die "GPS_ARCHIVE_ENCRYPTION_KEY missing or not a base64 32-byte value in $ENVFILE (R08 — retention is fail-closed without it)"
...
[ "$PRE" = "$EXPECT_MIGRATIONS" ] || die "pilot DB is at $PRE migrations, expected $EXPECT_MIGRATIONS (R09 changes nothing here)"
echo "  ok — pilot DB at $PRE migrations; GPS_ARCHIVE_ENCRYPTION_KEY present"

echo
echo "== 3/7  production baseline guard (captured now, re-checked at the end) =="
...
echo
echo "== 4/7  pre-deploy backup + off-box mirror (mandatory, fail-closed) =="
...
echo
echo "== 5/7  schema check (READ-ONLY — R09 adds no migration) =="
...
echo "$STATUS_OUT" | grep -q 'Database schema is up to date!' \
  || die "migrate status is not 'up to date' — the image's migration set does not match the pilot DB; investigate before deploying (this script never runs migrate deploy)"
...
echo "  migrations unchanged at $APPLIED (0 failed); image agrees with the pilot DB"

# ============================================================================================
echo
echo "== 6/7  swap (automatic rollback on any failure from here) =="
SWAP_STARTED=1
...
# ============================================================================================
echo
echo "== 7/7  verify (fail-closed — any failure rolls back) =="
FAILS=0
...
echo "  -- R09: touched pages render (unauthed — expect 2xx or a redirect to /login, never 5xx) --"
for p in /admin /admin/users "/admin/workforce?sort=ATTENTION" /worker; do
  c=$(http_code "$BASE$p"); echo "     $p -> $c"
  case "$c" in 5*|000) note_fail "$p http $c";; esac
done
...
echo "  -- R08: retention step still archive-gated and ran clean --"
...
# ============================================================================================
echo
echo "== production baseline guard — must be identical =="
...
echo
echo "DEPLOY OK — pilot on $IMAGE, DB at $EXPECT_MIGRATIONS migrations (unchanged)."
echo
echo "Post-deploy manual UI pass (R09 is UI-only — recommended), against the pilot:"
echo "  * /admin/users — type part of a name, Apply; filter by role + status; page next/prev"
echo "  * open a page you lack permission for — human 'no access' text, no permission code visible"
echo "  * /admin — 'documents needing attention' card (if any expiring quals) links to /admin/workforce"
echo "  * /admin tables + /admin header on a narrow window — no sideways page scroll"
echo "  * /worker — Check In / Check Out full cycle, GPS banners, switch-site sheet, status sheet"
echo "  * /admin/workers/<id> — breadcrumb + tab row; visit Profile / History / Locations"
echo "  Full checklist: docs/titanor-time/R09_BROWSER_ACCEPTANCE_RU.md"
echo
echo "Manual rollback later (back to $PREV_TAG; no schema change to undo):"
