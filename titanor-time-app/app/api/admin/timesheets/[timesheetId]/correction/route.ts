...
// Task A (2026-08-27) — one-shot "start an inline admin correction" for a SUBMITTED /
// FOREMAN_APPROVED timesheet: requestCorrection + openCorrectionDraft in sequence so the admin
// card can redirect straight to the editable /admin/corrections/[id] page instead of clicking
// through the PENDING -> DRAFT_OPEN step. Post-final corrections keep using POST /api/admin/
// corrections + the "Open draft" button.
...
export async function POST(request: NextRequest, { params }: RouteParams): Promise<NextResponse> {
  ...
  const { reason } = (rawBody && typeof rawBody === 'object' ? rawBody : {}) as { reason?: unknown };
  if (typeof reason !== 'string' || reason.trim().length === 0) {
    return jsonError(400, { code: 'VALIDATION_ERROR', message: 'Invalid request body.', fieldErrors: { reason: ['required'] } }, requestId);
  }

  const requested = await requestCorrection(timesheetId, authenticated.user.id, reason.trim(), requestId);
