...
import { createForemanAssignment, listForemanAssignments } from '@/lib/foreman-assignments';
...
export async function GET(request: NextRequest): Promise<NextResponse> {
  ...
  if (!(await hasPermission(authenticated.user.roles, 'foreman_assignment.read.all'))) {
  ...
  const result = await listForemanAssignments(page, pageSize);
