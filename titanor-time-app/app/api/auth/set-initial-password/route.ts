...
import { setInitialPassword } from '@/lib/activation';
import { SESSION_COOKIE_NAME, SESSION_DURATION_MS } from '@/lib/session';

export const dynamic = 'force-dynamic';
export const revalidate = 0;

// docs/titanor-time/04_ADMIN_FIRST_API_CONTRACTS.md §1 — POST /api/auth/set-initial-password.
// Public + valid ActivationToken, but still requires the CSRF header like every other mutating
// endpoint (owner instruction) — being unauthenticated doesn't exempt a POST from CSRF.
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  if (!(await checkRateLimit(`set-initial-password-ip:${ip ?? 'unknown'}`, IP_RATE_LIMIT.limit, IP_RATE_LIMIT.windowMs))) {
  ...
  const result = await setInitialPassword(token as string, password as string, requestId, ip, userAgent);
  ...
  // Cookie is only ever set here, after setInitialPassword's transaction has already committed
