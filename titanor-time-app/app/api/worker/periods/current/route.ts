...
import { getWorkerCurrentPeriod } from '@/lib/worker-context';

export const dynamic = 'force-dynamic';
export const revalidate = 0;

// docs/titanor-time/04_ADMIN_FIRST_API_CONTRACTS.md §9 — GET /api/worker/periods/current.
export async function GET(request: NextRequest): Promise<NextResponse> {
  ...
  const period = await getWorkerCurrentPeriod(authenticated.user.employeeId, helsinkiToday());
