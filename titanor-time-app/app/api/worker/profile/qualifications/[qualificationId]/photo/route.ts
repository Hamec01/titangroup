...
const ROUTE_TEMPLATE = '/api/worker/profile/qualifications/:qualificationId/photo';
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ qualificationId: string }> }): Promise<NextResponse> {
  ...
  // Same convention as the admin route above — path (here: the session's own employeeId +
  // qualificationId) fully defines the request, file bytes are not hashed.
  const requestHash = computeRequestHash({ pathParams: { employeeId: authenticated.user.employeeId, qualificationId } });
