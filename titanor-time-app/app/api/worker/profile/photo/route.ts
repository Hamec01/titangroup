...
export async function GET(request: NextRequest): Promise<NextResponse> {
  ...
  const photoPath = await getEmployeeProfilePhotoPath(authenticated.user.employeeId);
  if (!photoPath) {
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  const result = await setEmployeeProfilePhoto(authenticated.user.employeeId, file);
  if (!result.ok) {
...
export async function DELETE(request: NextRequest): Promise<NextResponse> {
  ...
  await removeEmployeeProfilePhoto(authenticated.user.employeeId);
  return NextResponse.json({ ok: true }, { status: 200, headers: successHeaders(requestId) });
}
