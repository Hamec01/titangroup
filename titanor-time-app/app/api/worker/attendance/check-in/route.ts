...
import { validateCheckInBody, performCheckIn } from '@/lib/attendance-clock';
...
// docs/titanor-time/T7A_1_ATTENDANCE_CLOCK_DESIGN.md §9.1/§12.2/§14 — POST /api/worker/attendance/check-in.
// employeeId always resolved from the session, never the body (§14). Idempotency-Key is OPTIONAL
// here (unlike geofence create) — ClockEvent.id (clientEventId) is the mandatory natural
// idempotency key regardless of whether this header is sent (§9.1a/§14).
const REQUIRED_CSRF_HEADER_VALUE = 'titanor-time';
const ROUTE_TEMPLATE = '/api/worker/attendance/check-in';
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  if (!(await hasPermission(authenticated.user.roles, 'attendance.clock.checkin.own'))) {
  ...
  if (!checkRateLimit(`actor:${authenticated.user.id}:${ROUTE_TEMPLATE}`, RATE_LIMIT.limit, RATE_LIMIT.windowMs)) {
    return jsonError(429, { code: 'RATE_LIMITED', message: 'Too many check-in attempts. Try again later.' }, requestId);
  }
  ...
  const validated = validateCheckInBody(bodyObject);
  if (!validated.ok) {
  ...
  const result = await performCheckIn(authenticated.user.employeeId, authenticated.user.id, requestId, validated.value);

  switch (result.kind) {
    case 'CREATED':
      return respond(201, result.body);
    case 'REPLAY':
      return respond(200, result.body);
    case 'CONFLICT':
      return respond(409, errorBody({ code: 'CLIENT_EVENT_ID_REUSED', message: 'This clientEventId was already used with a different payload.' }, requestId));
    case 'OUTSIDE_GEOFENCE':
      return respond(403, errorBody({ code: 'OUTSIDE_GEOFENCE', message: 'GPS reading is outside the site geofence.' }, requestId));
    case 'SITE_NOT_FOUND':
      return respond(404, errorBody({ code: 'SITE_NOT_FOUND', message: 'No site with this id.' }, requestId));
    case 'WORK_AREA_INVALID':
      return respond(400, errorBody({ code: 'VALIDATION_ERROR', message: 'Invalid request body.', fieldErrors: { workAreaId: ['not found for this site'] } }, requestId));
  }
}
