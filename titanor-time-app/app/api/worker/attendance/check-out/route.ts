...
import { validateCheckOutBody, performCheckOut } from '@/lib/attendance-clock';
...
// docs/titanor-time/T7A_1_ATTENDANCE_CLOCK_DESIGN.md §9.2/§12.2/§14 — POST /api/worker/attendance/check-out.
// employeeId always resolved from the session. Idempotency-Key is OPTIONAL — ClockEvent.id
// (clientEventId) is the mandatory natural idempotency key regardless.
const REQUIRED_CSRF_HEADER_VALUE = 'titanor-time';
const ROUTE_TEMPLATE = '/api/worker/attendance/check-out';
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  if (!(await hasPermission(authenticated.user.roles, 'attendance.clock.checkout.own'))) {
  ...
  if (!(await checkRateLimit(`actor:${authenticated.user.id}:${ROUTE_TEMPLATE}`, RATE_LIMIT.limit, RATE_LIMIT.windowMs))) {
    return jsonError(429, { code: 'RATE_LIMITED', message: 'Too many check-out attempts. Try again later.' }, requestId);
  }
  ...
  const validated = validateCheckOutBody(bodyObject);
  if (!validated.ok) {
  ...
  const result = await performCheckOut(authenticated.user.employeeId, authenticated.user.id, requestId, validated.value);
