...
export async function GET(request: NextRequest, { params }: RouteParams): Promise<NextResponse> {
  ...
  const siteIdInvalid = requireUuidParam(siteId, { code: 'SITE_REPORT_NOT_FOUND', message: 'No site report available for this id.' }, requestId);
  ...
  // Same shared function the admin route calls — scope is the only difference. Resolved inside
  // getSiteTimeReport's own REPEATABLE READ transaction, not pre-checked here.
  const outcome = await getSiteTimeReport(parsed.siteId, parsed.periodId, { page: parsed.page, pageSize: parsed.pageSize }, { kind: 'foreman', foremanUserId: authenticated.user.id, today });

  if (outcome.code === 'SITE_NOT_FOUND' || outcome.code === 'SITE_REPORT_NOT_FOUND') {
    // Uniform 404 for "doesn't exist" and "exists but not one of this foreman's current sites" —
    // never distinguishable, no enumeration oracle (task's own "FOREMAN scope" rule).
    return jsonError(404, { code: 'SITE_REPORT_NOT_FOUND', message: 'No site report available for this id.' }, requestId);
  }
