...
export async function GET(request: NextRequest): Promise<NextResponse> {
  ...
  const overview = await getForemanOverview(authenticated.user.id, authenticated.user.employeeId, helsinkiToday());
  return NextResponse.json(overview, { status: 200, headers: successHeaders(requestId) });
}
