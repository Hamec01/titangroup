...
// docs/titanor-time/01_SCREEN_MAP.md §4 `/foreman/review/[timesheetId]/approve` — POST
// /api/foreman/review-scopes/:reviewScopeId/approve. Reuses lib/review-scopes.ts's
// approveReviewScope core (already built for the ADMIN fallback path) — the only thing added
// here is the own-site ownership gate before calling it.
...
export async function POST(request: NextRequest, { params }: RouteParams): Promise<NextResponse> {
  ...
  if (!(await hasPermission(authenticated.user.roles, 'timesheet.foreman_review'))) {
